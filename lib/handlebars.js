    ...
    this.render = function (subDir,name, data, report) {
        return this.getTemplate(subDir,name)
            .then((template) => {
                    ...
                    rtOptions.helpers["graph"] = graphPosition(report,rtOptions)
...
function graphPosition(report,rtOptions){ // block helper
    return function( context,  options) {
        ...
        let graphHeight = 100
        let points = ds.rows()
        let max = ds.accumulator().max("posts")
        let rowNr = context.rowNr
        let value = context.data.posts.value
        console.log("graph " +points + " "+max+" "+rowNr+" "+context.column)
        let coord ={
            x1:(graphWidth/points)*rowNr,
            x2:(graphWidth/points)*(rowNr+1),
            y1: graphHeight - ((graphHeight/max)*value),
            y2: graphHeight - ((graphHeight/max)*value)
        }
